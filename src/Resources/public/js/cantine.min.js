...
l);a(".state span")[0].innerText=c;e+f==b&&n()}})})})().then(()=>{Swal.fire({title:"Envois termin\u00e9s",html:`${e} r\u00e9ussite(s) pour ${f} \u00e9chec(s)`});a("#envoiEnCours").hide()})}})});a("a.paiement").on("click",function(d){d.preventDefault();let b=d.delegateTarget.href.match(/id=([^&]*)/)[1];Swal.fire({icon:"question",text:"Quel moyen de paiement a \u00e9t\u00e9 choisi ?",input:"select",inputOptions:{esp:"Esp\u00e8ce",chk:"Ch\u00e8que",vir:"Virement"},confirmButtonText:"Valider"}).then(c=>
{1==c.isConfirmed&&a.ajax({url:"/ajax.html",data:{action:"majPaiement",item:b,choix:c.value},type:"POST",success:function(e){e=a.parseJSON(e);"error"==e.result?(alert("Erreur lors du paiement"),console.error(e)):0<a("form#genFacture").length?a("form#genFacture").submit():document.location.href=document.location.href}})})});a("form#genFacture input[name=dateDebut]").on("change",function(){let d=moment(this.value),b=moment(this.value);d.date(1);b.endOf("month");this.value=d.format("YYYY-MM-DD");a("input[name=dateFin]").val(b.format("YYYY-MM-DD"))})});